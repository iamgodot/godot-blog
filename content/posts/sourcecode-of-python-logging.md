...
阅读源码应有目的性，除了抱着学习的心态，我对 Python 的 Logging 有不少疑惑：
...
虽然看代码也不一定都能解决，不过总是个好的开始。首先还是要熟悉 Logging 的用法，不然如同乱翻书。
...
除了代码，Logging 的官方文档也介绍了许多用法和 Best practice。

如果想在应用运行过程中更改配置，那么一种简单的实现是使用 `fileConfig` 并开启额外的端口来监听文件内容的变化。当然如果从更高层的角度来考虑，解决办法还有很多，比如在 Redis 中保存配置，再单独启动一个 Worker 来订阅更新，如果有改变则直接以代码的方式 Reconfig。
...
那么，到底选择 Logging 还是 `print`？除了[这里](https://docs.python-guide.org/writing/logging/#or-print)提到的，如果涉及到多线程的场景，前者会更安全一些。而 `print` 简单好用，还不需要额外 `import`。所以很简单，一切选择都应当以实际场景为准，没有绝对的好坏和对错。
